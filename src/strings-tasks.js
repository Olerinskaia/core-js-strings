 ...
 *
 * @param {+ value2string} value - The input string to remove trailing whitespaces from.
...
function sumOfCodes(str) {
  return str ? [...str].reduce((a, e) => a + e.charCodeAt(0), 0) : 0;
}
...
function countVowels(str) {
  return str.match(/[aeiouy]/gi)?.length ?? 0;
}
...
function isPalindrome(str) {
  const onlyLetters = str.replace(/[^a-zA-Z]/g, '').toLowerCase();
...
function findLongestWord(sentence) {
  ...
  return words.reduce(
    (acc, nextWord) => (nextWord.length > acc.length ? nextWord : acc),
    ''
  );
}
